...
get '/threads/:threadId' do |thread_id|
  result = api_client.execute(:api_method => gmail_api.users.threads.get,
                              :parameters => {userId: 'me', id: thread_id, fields: 'messages(id,threadId,snippet,labelIds)'},
                              :authorization => user_credentials)
  [result.status, {'Content-Type' => 'application/json'}, result.data.to_json]
end

get '/messages/:msgId' do |msg_id|
  result = api_client.execute(:api_method => gmail_api.users.messages.get,
                              :parameters => {userId: 'me', id: msg_id, fields: 'id,threadId,snippet,labelIds,payload(headers)'},
                              :authorization => user_credentials)
  [result.status, {'Content-Type' => 'application/json'}, result.data.to_json]
end
